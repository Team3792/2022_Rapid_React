...
public class TalonFXSimProfile extends SimProfile{
    private final WPI_TalonFX _falcon;
     ...
     */
    public TalonFXSimProfile(final WPI_TalonFX falcon, final double accelToFullTime, final double fullVel, final boolean sensorPhase) {
        this._falcon = falcon;
    ...
    public void run() {
        ...
        double theoreticalVel = outPerc * _fullVel * PhysicsSim.random(0.95, 1);
        // Simulate motor load
        ...
        double supplyCurrent = Math.abs(outPerc) * 30 * PhysicsSim.random(0.95, 1.05);
        ...
        _falcon.getSimCollection().setBusVoltage(12 - outPerc * outPerc * 3/4 * PhysicsSim.random(0.95, 1.05));
    }
}
